...
%token ADD SUB MUL DIV EXCLAMATION MORE OR AND LESS ASSIGN EQUAL NOEQUAL LESSEQUAL MOREEQUAL PERC PLUSPLUS MINUSMINUS PRE_INCREMENT POST_INCREMENT PRE_DECREMENT POST_DECREMENT
%token RETURN
%token LINECOMMENT COMMENTBEIGN COMMENTELEMENT COMMENTLINE COMMENTEND

%nterm <stmttype> Stmts Stmt AssignStmt BlockStmt IfStmt ReturnStmt DeclStmt FuncDef WhileStmt ConstDeclStmt SignleStmt ForStmt OptDecl
...
UnaryExp
    ...
    | PLUSPLUS LVal {
        SymbolEntry *se = new TemporarySymbolEntry(TypeSystem::intType, SymbolTable::getLabel());
        $$ = new SignleExpr(se, SignleExpr::PRE_INCREMENT, $2); 
    }
    | MINUSMINUS LVal {
        SymbolEntry *se = new TemporarySymbolEntry(TypeSystem::intType, SymbolTable::getLabel());
        $$ = new SignleExpr(se, SignleExpr::PRE_DECREMENT, $2); 
    }
    | LVal PLUSPLUS {
    SymbolEntry *se = new TemporarySymbolEntry(TypeSystem::intType, SymbolTable::getLabel());
    $$ = new SignleExpr(se, SignleExpr::POST_INCREMENT, $1); // 后置++操作
    }
    | LVal MINUSMINUS{
    SymbolEntry *se = new TemporarySymbolEntry(TypeSystem::intType, SymbolTable::getLabel());
    $$ = new SignleExpr(se, SignleExpr::POST_DECREMENT, $1); // 后置--操作
    }
    ;
...
OptDecl
    : Type ID ASSIGN Exp { 
        ...
        $$ = new DeclStmt(new IdList({new Id(se)}, {new AssignStmt(new Id(se), $4)}));
        delete []$2;
    }
    ;
...
FuncDef
    ...
    ;
ForStmt
    : FOR LPAREN { identifiers = new SymbolTable(identifiers); }
      OptDecl SEMICOLON Cond SEMICOLON UnaryExp RPAREN 
      Stmt 
      {
        $$ = new ForStmt($4, $6, $8, $10);
        SymbolTable *top = identifiers;
        identifiers = identifiers->getPrev();
        delete top;
      }
